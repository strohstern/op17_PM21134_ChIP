...
subtitle: "Analysis of ChIP - QC (runs PM22166 and DN19154)"
...
```{r}
RMD_file_name <- "5-2b.ChIP_QC_SMARCB1_PM22166_DN19154"
```
...
# Alternative approach to identify consensus peaks

Rather than using the standard approach used above to identfiy consensus peaks, we used the Bioconductor package [DiffBind](https://bioconductor.org/packages/release/bioc/html/DiffBind.html).
Using peaks that occur in at least 2 peak sets to generate consensus regions, `DiffBind` takes the number of uniquely aligned reads to compute a normalised read count for each sample at every potential binding site. The peaks in the consensus peakset are re-centred and trimmed based on calculating their summits (point of greatest read overlap) in order to provide more standardized peak intervals.
As the most common peak interval size in this dataset is 1000, we chose `summit = 500`. Note that using summits assumes that the summit is a good representation of the peak. 
...
diffbind_cons_peaks <-
  read_tsv(
    here(
      ...
      "derived_data",
      "Tables",
      "5-3.ChIP_DiffBind_SMARCB1_PM22166_DN19154",
      "consensus_peaks_DiffBind.txt"
    )
  )
```

With this approach we identified the `r NROW(diffbind_cons_peaks)` consensus peaks. 

The peaks identified with DiffBind show the following overlap:
```{r fig.height=8, message=FALSE, warning=FALSE}
diffbind_cons_peaks %>% 
  mutate(interval_id = str_c("interval", rownames(.), sep = "_"),
    peak_id = str_c(seqnames, start, end, interval_id, sep = )) %>%
  mutate_at(vars(contains("bool")), ~ as.integer(.)) %>%
  as.data.frame() %>%
  upset(
    sets = (
      diffbind_cons_peaks %>% dplyr::select(contains("bool")) %>% colnames()
    ),
    order.by = "freq",
    keep.order = TRUE
  )
...
An excel file with all the peaks (standard and DiffBind approach), in addition to the bed file with the DiffBind consensus peaks, can be downloaded from the following link:
...
openxlsx::write.xlsx(
  list(
    "SMARCB1_consensus_peaks_full" = SMARCB1_consensus_peaks %>%
      ...
      relocate(peak_id),
    "SMARCB1_consensus_DiffBind" = diffbind_cons_peaks %>%
      mutate(peak_id = str_c(seqnames, ":", start, "-", end, sep = "")) %>% 
      relocate(peak_id)
  ),
...
diffbind_cons_peaks %>%
  mutate(interval_id = str_c("interval", rownames(.), sep = "_"),
         score = 0) %>%
  select(seqnames, start, end, interval_id, score, strand) %>%
  write_tsv(
    str_c(
      output_dirs[["Tables"]],
