...
read_tsv(
  ...
  map(~ {
    col_pal <- .x %>%
      select(sample_id) %>%
      distinct() %>%
      mutate(inputs = case_when(str_detect(sample_id, "Input") ~ "Input",
                                TRUE ~ "Not")) %>%
      group_by(inputs) %>%
      group_split() %>%
...
Based on the Fingerprint plots above, we see good enrichment for the two Histone marks, slight enrichment for ASCL1 and nearly no enrichment of SMARCC1 in comparison to the input controls. This indicates that the confident identification of peaks/binding sites for SMARCC1 in this dataset may not be possible.
...
All 4 used antibodies seem to preferentially bind to the promoter regions, though there is hardly any enrichment for SMARCC1.
...
In line with the above noticed poor enrichment in SMARCC1, only few consensus peaks were detectable with the SMARCC1 antibody.
...
read_delim(here(broad_dir, "qc", "macs_annotatePeaks.summary.txt")) %>%
  ...
  filter(!str_detect(sample, "ASCL1|SMARCC1")) %>% 
  mutate(antibody = str_extract(sample, "H3K27ac|H3K4me3")) %>% 
  ggplot(aes(
    fill = factor(
      annotation,
      levels = c(
...
read_delim(here(narrow_dir, "qc", "macs_annotatePeaks.summary.txt")) %>%
  pivot_longer(cols = !sample,
               ...
               values_to = "peak_count") %>%
  filter(str_detect(sample, "ASCL1|SMARCC1")) %>% 
  mutate(antibody = str_extract(sample, "ASCL1|SMARCC1")) %>% 
  ggplot(aes(
    fill = factor(
      annotation,
      levels = c(
...
raw_counts_SMARCC1 <-
  read_tsv(
    here(
      ...
      "consensus",
      "SMARCC1",
      "SMARCC1.consensus_peaks.featureCounts.txt"
    ),
    skip = 1
  ) %>% 
  rename_with( ~ str_remove(., ".mLb.clN.sorted.bam")) %>%
  mutate(interval_id = str_c(Geneid, Chr, Start, End, Strand, sep = "_")) %>%
  select(interval_id, contains("SMARCC1")) %>%
   column_to_rownames(var = "interval_id")

meta_data_SMARCC1 <- meta_data %>%
  filter(str_detect(antibody, "SMARCC1")) %>% 
  select(sample_id, group, replicate, genotype) %>%
  mutate(across(group:genotype, ~ as_factor(.))) %>%
  column_to_rownames(var = "sample_id")
...
# DESeq object
dds_peaks_SMARCC1 <- DESeqDataSetFromMatrix(countData = raw_counts_SMARCC1,
                                    colData = meta_data_SMARCC1[colnames(raw_counts_SMARCC1),],
                                    design = ~ genotype)

# normalized peak counts
dds_peaks_SMARCC1 <-  estimateSizeFactors(dds_peaks_SMARCC1)

rlog_peaks_SMARCC1 <- rlog(dds_peaks_SMARCC1)
plotPCA(rlog_peaks_SMARCC1, intgroup = c("genotype", "replicate"))  +
  guides(col = guide_legend(ncol = 2)) +
  ggtitle("PCA plot of reads in consensus peaks for SMARCC1")
```
...
openxlsx::write.xlsx(
  list(
    "ASCL1_consensus_peaks_full_info" = ASCL1_consensus_peaks %>%
      ...
      relocate(peak_id),
    "SMARCC1_consensus_peaks_full" = SMARCC1_consensus_peaks %>%
      mutate(peak_id = str_c(chr, ":", start, "-", end, sep = "")) %>% 
      relocate(peak_id),
